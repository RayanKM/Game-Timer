...
import com.watch.cardtimer.database.ChessClockDatabase
import com.watch.cardtimer.fragments.settings.SettingsFragment.Companion.ALERT_TIME_KEY
import com.watch.cardtimer.fragments.settings.SettingsFragment.Companion.LOW_TIME_WARNING_KEY
import com.watch.cardtimer.fragments.settings.SettingsFragment.Companion.PREFERENCES_NAME
import com.watch.cardtimer.fragments.settings.SettingsFragment.Companion.SOUND_AFTER_MOVE_KEY
import com.watch.cardtimer.fragments.settings.SettingsFragment.Companion.VIBRATE_KEY
import com.watch.cardtimer.utils.ChessUtils.Companion.CURRENT_CLOCK_KEY
import com.watch.cardtimer.utils.CountDownTimer
import com.watch.cardtimer.utils.CountDownTimer.Companion.NOT_STARTED
import com.watch.cardtimer.utils.CountDownTimer.Companion.ONE_MINUTE
import com.watch.cardtimer.utils.CountDownTimer.Companion.ONE_SECOND
import com.watch.cardtimer.utils.CountDownTimer.Companion.PAUSED
import com.watch.cardtimer.utils.CountDownTimer.Companion.RUNNING
...
class ClockViewModel(application: Application) : ViewModel() {
    ...
    val timeLeftString1: LiveData<String> = timeLeft1.map {
    ...
    val timeLeftString2: LiveData<String> = timeLeft2.map {
        /* elapsedSeconds passed this way allows to set times like
            14sec, 56millis (which it's shown as 00:14) to be 15sec,55millis
            (which it's shown as 00:15). So when the time is 0sec, n-millis, it will
            show 00:01 until the time is 0sec 0millis when it'll be 00:00 (time up)
    ...
    val guidelinePercentage: LiveData<Float> = turn.map {
        when (it) {
            TURN_1 -> PERCENT_66
            TURN_2 -> PERCENT_33
            else -> PERCENT_50
